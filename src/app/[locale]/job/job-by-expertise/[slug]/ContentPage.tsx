...
import { expertises, levelList, workingFormList } from "@/config/variable"
...
export const ContentPage = ({ slug, locale }: JobDetailClientProps) => {
  ...
  const t = useTranslations('JobByExpertisePage');

  const { data, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/job/job-by-expertise/${slug}?locale=${locale}&level=${level}&workingForm=${workingForm}&page=${page}`,
    fetcher,
    {
  ...
  const { data: totalPageData } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/job/job-by-expertise/total-pages/${slug}?locale=${locale}&level=${level}&workingForm=${workingForm}`,
    fetcher,
    {
      keepPreviousData: true,     // Giữ data cũ khi đổi key
      revalidateOnFocus: false,   // Không fetch lại khi focus tab
    ...
    <>
      <div className="container mx-auto px-[16px]">
        ...
        <h2 className="font-[700] text-[28px] text-[#121212] mb-[10px] flex items-center">
          {totalRecord ?? 0} {t('job_in')} {expertiseName}
        </h2>
        {/* filter */}
        <div
